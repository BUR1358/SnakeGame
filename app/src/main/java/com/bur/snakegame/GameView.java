...
public class GameView extends View {
    ...
    public static int sizeOfMap = 80 * Constants.SCREEN_WIDTH / 1080; //80-отвечает за РАЗМЕР КАРТЫ.
    ...
    public GameView(Context context, @Nullable AttributeSet attrs) {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                if ((i + j) % 2 == 0) {
                    arrGrass.add(new Grass(bmGrass1, j * sizeOfMap + Constants.SCREEN_WIDTH / 2 - (w / 2) * sizeOfMap, i * sizeOfMap + 100 * Constants.SCREEN_HEIGHT / 2220, sizeOfMap, sizeOfMap));
                } else {
                    arrGrass.add(new Grass(bmGrass2, j * sizeOfMap + Constants.SCREEN_WIDTH / 2 - (w / 2) * sizeOfMap, i * sizeOfMap + 100 * Constants.SCREEN_HEIGHT / 2220, sizeOfMap, sizeOfMap));
                }
            }
        }
